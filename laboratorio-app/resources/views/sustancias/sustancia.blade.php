    ...
    <section>
        @foreach ($informacion as $item)
            <div class="card" style="width: 20%; margin-top:20px; margin-bottom: 20px; ">
                <img src="{{ route('getImagen', $item->imagen_reactivo) }}" alt="imagen sustancia"
                ...
                <div class="card-body">
                    <a href="{{ route('getPdf', $item->ficha_seguridad) }} " target=" _blank" class="card-link">Ficha
                        seguridad</a>
                </div>
                <a href="{{ route('viewEditar', $item->codigo_laboratorio1) }}"type="button"
                    class="btn btn-outline-warning" style="margin-bottom: 10px">Utilizar sustancia</a>
                <a href="{{ route('viewEliminar', ['id' => $item->codigo_laboratorio1, 'id_sustancia' => $item->id]) }}"
                    type="button" class="btn btn-outline-danger" style="margin-bottom: 10px">Eliminar sustancia</a>
            </div>
        @endforeach
    </section>
